...
2. Each beat is one step of the explanation: what this part does and why the block needs it, in plain words a reader who never saw the code understands. Use the real names of variables and functions when they help the reader. One line is usually enough; use two lines (split with \\n) when one line cannot carry the step.
3. Top beat to bottom beat reads as one story: the block's work from its input to its result. Merge adjacent cells into one beat when they serve one step; split where the story turns.
4. A beat holds all of a statement or none of it. Never cut a loop or a branch in the middle.
5. A small helper deserves an honest explanation of what it is for, not a mechanical restatement of its lines.
6. When the lines call a compiled function from the list above, speak at its level: use its one-line meaning, do not re-explain its inside.

Root rules:
7. root.text is the signature of the block: name(arguments) -> result, then one clause after " — " that says what the whole block does for its caller. The top-level script block starts with the word script instead of a signature.
8. root.takes and root.gives are short noun phrases that name the input and the output.
