...
Two operations: **explain** and **map**. The state is one `human/` folder at the project root, created once with `human init`: one JSON map per code file, `explanation_<path>.json` with the `/` of the path written as `__`, and one map for the project itself, `human/human.json` — one entry whose pins point at the files, so a reader who opens the project has a top, plus the list of the project's files. Two more kinds of map hold a telling with no code under it: `human/project.json`, the telling of the whole, and `human/human_<name>.json`, a human file — a telling over some of the files, as many as the user wants, each made and named by the user. A file is named by its path from the root, like `src/app.py`; the project map is named by the word `project`, and a human file by its bare name, like `payments`. The human CLI owns the `human/` folder — never edit a map by hand; every operation you need is a human command (`init`, `map`, `retext`, `undo`, `show`, `lines`, `sync`, `serve`, `train`). The map grows delta by delta: every map run appends one entry and never changes the entries before it. When a training session is open, the explain step writes three versions instead of one — see §7.
...
- `[the gates](src/cmd_map.py:e1:the checking)` — the words point at **an anchor of an entry of another file's map**: entry 1 of `src/cmd_map.py`, its anchor `the checking`. This pin lives in a map with no code under it only — the project map or a human file (§8).
- `[the money side](payments)` — the words point at **a human file on its own**, by its bare name. In the reader the pin leads to that map's first entry.
- `[the adding of the amounts](payments:e1:the sum)` — the words point at **an anchor of an entry of another map with no code under it**: entry 1 of the human file `payments`, its anchor `the sum`. `project` stands in the same place for the project map.
...
- An `e<id>:` target must name an existing entry and existing anchor words inside it. It cannot make a circle — and the check runs over the whole graph of maps, not one map alone, so a human file that stands on a human file that stands on the first is refused. An `e<id>:` target never crosses a file border — a plainer telling stays inside its file, so a change in one file makes that file's map stale, and every map with no code under it whose pins reach it.
...
- The pins of `human/human.json` are file and `file:block` pins only. human refuses the rest. A map with no code under it — `human/project.json` and every `human/human_<name>.json` — takes file, `file:block`, `file:e<id>:words`, `<name>` and `<name>:e<id>:words` into another map with no code under it, and `e<id>:` into its own entries.
...
- `human map project` maps the project telling of §8: the entry goes into `human/project.json`, takes no `--block`, and its pins may be file, `file:block`, `file:e<id>:words`, `<name>` and `<name>:e<id>:words` into another map with no code under it, and `e<id>:` into an earlier entry of the same map.
- `human map <bare name>` maps a human file of §8: the entry goes into `human/human_<name>.json`, takes no `--block`, and takes the same pins as the project telling. The map is made on the first run, so the name alone opens it. The name is one word, with no folder and no suffix; a file at the root that answers to the same word wins the name, and `human init` warns when one does.
...
- Entries that point into a repaired entry are marked stale — in the file's map, and in every map with no code under it, the project map and every human file, when one of its entries pins the repaired entry. Each note names the map it waits on, so the repair is `human sync <that name> --stale <id>`.
...
`human sync <bare name>` does for a human file what `human sync project` does for the project map: the old state is git `HEAD`, the deterministic pass re-resolves every pin, and one claude call mends the entries that pin a changed file. `human sync <bare name> --stale <id>` repairs an entry a parent made stale, wherever that parent lives — a file's map, the project map, or another human file.
...
After a retext, an undo, or a sync, report what changed and confirm with `human show <code_file>`. `human show project`, and `human show <bare name>` for a human file, report that map's entries, their warnings, and the coverage as files with a pin out of the files of the project.

## 7. Train

The sessions live in the training store — an S3 bucket behind a Lambda, one JSON object per session under the user's id, each stamped with the project id `human init` wrote into `human/human.json`, so a session belongs to one user and one project — and never on disk: every `train` call, the feed, and the pick read and write there. A key names the user; `human login <key>` once per machine writes it to `~/.config/human/credentials.json`, and without it `human train` refuses and says to log in. `human train --open` starts a session; it stays open until `human train --close`. A retext or a sync that makes an open row follow warns and goes on when the store does not answer; an undo that must check the open session refuses instead. A reader write that asks claude for a telling — a decompile, a create, an expansion without words — opens a session by itself when none is open, and the event carries its id. While a session is open, the explain step of §2 writes **the versions** of the same abstraction — two for a new abstraction, three for a sync — and registers them with `human train`, one row per file and per target. The user reads them side by side in the reader: an open session shows as a layer over `/human/web.html`, one row per file, the versions as cards to swipe; a hide button puts it away, a chip in the head bar with the counts brings it back, a new row brings it back on its own, and a "close training" button applies the picks. Nothing goes into the map before the close.
...
- `--entry <id>` when the versions refine an entry that exists; the close runs `human retext`. `--block <name>` when the versions zoom on one block; the close runs `human map --block`. Neither: a first entry of an unmapped file, or a plainer layer whose pins are `e<id>:`. `--target <id> --words <text>` when the versions expand the highlighted words of an entry: the text must not pin into the target, and the close queues a `link` event after the map, so claude pins the words of the target down into the new entry.
- The first call for a file makes the row and takes a full copy of the code; the next calls with the same `--entry`, `--block` or `--target` fill the other slots, and a call with other options makes another row of the same file. A row of a map with no code under it — `human train project`, or `human train <bare name>` for a human file — takes no `--block`, because such a map has no blocks of its own, and copies in place of a file's text the files under it: every file of the project for the project map, the files its pins reach for a human file, and for a sync row the changed ones with their diff. Every call checks the anchors like `human map`, so a picked text can always be applied. Every call ends with the filled slots and, when one is missing, an `empty:` line — write that slot before the report; the close names every row that still has an empty slot.
...
`human train --close`, on the user's word or from the layer's button, applies every picked row — `map` or `retext` — writes the new entry id into the row, and marks the session finished. A create row without a pick takes best, or free when best is missing, and the row says the pick was a default: a reader write always lands. A sync row without a pick carries over, because the current text already stands in the map: the next `human train --open` rebuilds it against the code and the map of that day, keeps every version whose anchors still hold, and drops the rest — write the dropped versions again. When one apply fails, the session stays open; correct that row and close again.

## 8. The project and the human files
...
A **human file** is the same kind of map, on a smaller piece of the project: a telling over the files the user chooses — one part of the tool, one road through it — with no code of its own. It lives in `human/human_<name>.json` and answers to its bare name everywhere a file path goes: `human map payments`, `human show payments`, `human retext payments 1`, `human sync payments`, `human undo payments`. The user makes one from the reader with the "new human file" line, or the name alone makes it on the first `human map`.

- The pins are the ones of the project telling, and two more: `[the words](payments)` at a human file as a whole, and `[the words](payments:e1:the sum)` at one anchor of one of its entries. The project map and a human file reach each other the same way.
- A human file may stand on another human file, and that one on a third: the layers cross the maps. The chain never turns back on itself — the cycle check runs over the whole graph of maps.
- The stale road is the one of §5: when a parent changes, in a file's map or in a map with no code under it, every entry that pins it gets a note that names the map and the entry it waits on.
