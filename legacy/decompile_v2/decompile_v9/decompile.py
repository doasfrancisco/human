...
Story rules:
12. story is the explanation a person reads; the beats are only the bridge to the code. Its shape, in this order: first ONE sentence that says what the block does; then, when it helps, one small worked example — a tiny concrete input and what the block makes of it, with real values that stay consistent; then at most 3 short sentences for rules and edge cases the example does not show. Shorter is better.
13. Story sentences have 20 words or fewer, one idea per sentence, active voice. No metaphors: never words like carve, glue, promise, walk.
14. Every technical term the story uses is a name from the code, or is defined: in the story at first use, or in terms with a plain one-line meaning.
15. terms lists those defined words as {{"term": ..., "meaning": ...}}. Each term appears verbatim in the story. An empty list is legal.

A gold story, for a function digits(text) that collects the digit characters of a string as ints. Imitate this shape — claim, tiny example, short rules:
digits(text) turns a string into the list of its digits.
