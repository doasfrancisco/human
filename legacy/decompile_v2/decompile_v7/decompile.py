...
TERMS = """You are a decompiler. The map of one code file is done: every block has one signature text and pseudocode stage texts. Some texts coin a noun phrase for a thing the code computes but never names with a function or a variable. Collect these terms and define each one.
...
def check_pass(m, cells, stmts, blank, ess, bad, flow):
    ...
    for g in groups:
        ...
        got -= blank
        if b > a and not flow and not any("text" in c for c in cells):
...
def one_pass(cells, name, code, lines, stext, stmts, blank, ess, layer, label, span, ctx, bad, flow, final):
    ...
    for attempt in range(8):
        try:
            m = ask_claude(base + note) if flow else ask(base + note)
...
def compile_block(bl, cells, name, code, lines, stmts, blank, ess, ctx, bad, flow_only):
    ...
    while len(cells) > 1 or "takes" not in cells[0]:
        ...
        if flow_only:
            flow = not flowed and len(cells) > 1
            final = staged and not flow
        else:
            flow = staged and not flowed and len(cells) >= 4
            final = staged and not flow
        cells = one_pass(cells, name, code, lines, stext, bstmts, blank, ess, layer, label, span, ctx, bad, flow, final)
...
def main():
    args = sys.argv[1:]
    flow_only = "--flow" in args
    if flow_only:
        args = [a for a in args if a != "--flow"]
    path = Path(args[0])
    ...
    for bl in sorted(blocks, key=lambda b: (lay[b["name"]], b["lines"][0][0])):
        ...
        root = compile_block(bl, parts[bl["name"]], path.name, code, lines, stmts, blank, ess, ctx, bad, flow_only)
        root["name"] = bl["name"]
