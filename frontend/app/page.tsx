...
export default function Home() {
  ...
  if (!showContextCard && selected === "context") {
    setSelected(files.contextRole === "direct" || files.status.hasPython ? "python" : "human");
  }
      ...
      (range) =>
        !ranges.some(
          (other) =>
            ...
            other.to <= range.to &&
            other.to - other.from < range.to - range.from
        )
    );
  }
  ...
  function handleHumanModClick(pos: number) {
    ...
    if (!clicked.length) {
      setHighlightedLines([]);
      setMessage(`No ${provenanceLabel} provenance found for that human span`);
      return;
    }
  ...
  function handleContextModClick(line: number) {
    if (files.contextRole !== "split") return;
    const entry = files.contextProvenance.find((item) => Number(item.line) === line && item.target);
    if (!entry?.target) {
      setHighlightedLines([]);
      setMessage("No split unit found for that context line");
      return;
    }
    const lines = linesForTarget(entry.target);
    setHighlightedLines(lines);
    setMessage(`Highlighted ${lines.length} .context line${lines.length === 1 ? "" : "s"} for ${entry.target}`);
  }

  function contextModClickHandler() {
    if (selected === "human") return ({ pos }: { pos: number }) => handleHumanModClick(pos);
    if (selected === "context" && files.contextRole === "split") {
      return ({ line }: { pos: number; line: number }) => handleContextModClick(line);
    }
    return undefined;
  }
    ...
    <main className="flex min-h-screen bg-zinc-950 text-zinc-100">
      <aside className="flex w-[330px] shrink-0 flex-col border-r border-zinc-800 bg-zinc-950">
        <div className="border-b border-zinc-800 px-4 py-4">
          <div className="mb-4 flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900 py-1.5 pl-3 pr-1.5">
            <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">project</span>
            <span className="min-w-0 flex-1 truncate text-sm font-semibold text-zinc-100">
              {activeProject ? activeProject.name : "—"}
            </span>
            <Link
              href="/projects"
              className="shrink-0 rounded-md px-2 py-1 text-xs font-semibold text-indigo-300 transition hover:bg-indigo-500/10"
            >
              Projects
            </Link>
          </div>
        ...
        <div className="border-b border-zinc-800 p-3">
          ...
          {showContextCard ? (
            <SidebarFile
              ...
              dirty={contextDirty}
              status={humanDirty ? "waiting for .human" : "generated"}
              badge={files.contextRole ? <RoleBadge role={files.contextRole} /> : null}
          ...
          {files.tree.length === 0 ? (
            ...
            <TreeView
              ...
              onCheckout={(kind, hash, target) =>
                run("checkout", () => checkout(kind, hash), `Checked out ${kind}`, (nextFiles) =>
                  setHighlightedLines(target ? linesForTarget(target, nextFiles.contextProvenance) : [])
                )
              }
            />
          )}
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <header className="border-b border-zinc-800 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            {selected === "context" && files.contextRole ? <RoleBadge role={files.contextRole} /> : null}
          </div>
      ...
      {pendingDelete ? (
        <ConfirmDeleteModal
          ...
          detail={hashShort(pendingDelete.hash)}
          disabled={disabled}
      ...
      return (
        <div className="icon-action-row">
          ...
          <ActionIconButton
            ...
            label="Generate .context (auto leaf / split)"
          ...
          <ActionIconButton
            ...
            disabled={disabled || !drafts.human.trim()}
            onClick={() =>
              run("compile", () => compile(drafts.human), "Compiled", (nextFiles) =>
        ...
        <PendingPane
          ...
          onClick={() =>
            files.contextRole === "direct"
              ? run("compile", () => compile(drafts.human), "Compiled .py", () => setSelected("python"))
              : run("context-to-python", () => contextToPython(drafts.context), "Generated .py", () => setSelected("python"))
          }
        />
      );
    }
    ...
    if (selectedDirty) {
      return (
        <DirtyEditor
          ...
          actions={fileActions()}
          highlightedLines={selected === provenanceKind ? highlightedLines : []}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <span className="flex items-center gap-2 font-mono text-sm font-semibold text-zinc-100">
          {title}
          {badge}
        </span>
                ...
                <div className="ml-10 mt-2 space-y-2 border-l border-zinc-800 pl-3">
                  {contexts.map((context) => {
                    ...
                    if (context.role === "direct") {
                      return (
                        <div key={context.hash} className="space-y-2">
                          {pythons.length ? (
                            pythons.map((python) => (
                              <TreeButton
                                key={python.hash}
                          ...
                          <div className="ml-4 mt-2 space-y-2 border-l border-zinc-800 pl-3">
                            {pythons.map((python) => (
                              <TreeButton
                                key={python.hash}
                                active={python.active}
                                label={python.target ? `.py · ${python.target}` : ".py"}
                                preview={python.preview}
                                hash={python.hash}
                                disabled={disabled}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 text-xs">
        <span className="flex items-center gap-2 font-mono font-semibold text-zinc-300">
          {label}
          {badge}
        </span>
